...
class Viterbi(object):
    ...
    def init_decoding(self, frame_scores):
        ...
        for label in self.grammar.possible_successors(context):
            ...
            # FIXME: changed here
            score = self.grammar.score(context, label, self.length_model.mean_lengths) + self.frame_score(frame_scores, self.frame_sampling - 1, label)
    ...
    def decode_frame(self, t, old_hyp, frame_scores):
        ...
        for key, hyp in old_hyp.items():
            ...
            for new_label in self.grammar.possible_successors(context):
                ...
                # FIXME: change that
                score = hyp.score + self.frame_score(frame_scores, t, label) + self.length_model.score(length, label) + self.grammar.score(context, new_label, self.length_model.mean_lengths)
    ...
    def finalize_decoding(self, old_hyp):
        ...
        for key, hyp in old_hyp.items():
            ...
            score = hyp.score + self.length_model.score(length, label) + self.grammar.score(context, self.grammar.end_symbol(), self.length_model.mean_lengths)
